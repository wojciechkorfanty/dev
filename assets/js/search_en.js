  ...
  function displaySearchResults(results, store) {
    ...
    if (results.length) { // Are there any results?
      ...
      for (var i = 0; i < results.length; i++) {  // Iterate over the results
        ...
        appendString += '<p>Authors: ' + item.author + '</p>';
        appendString += '<blockquote cite="' + item.url + '"><p class="content_en">„' + item.content + '...“</p></blockquote>';
        appendString += '<p>Keywords: ' + item.category + '</p>';
        appendString += '<aside class="group"><a href="/studies_en_' + item.group + '.html#' + item.article + '"><div data-icon="ei-chevron-right" data-size="s"></div><p>' + item.groupname + '</p></a></aside>';
